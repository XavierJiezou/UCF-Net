...
def cat_keep_shapes(x_list: List[Tensor]) -> Tuple[Tensor, List[Tuple[int, ...]], List[int]]:
    """
    Flatten and concatenate tensors while keeping track of their shapes and token counts.
    """
...
def uncat_with_shapes(flattened: Tensor, shapes: List[Tuple[int, ...]], num_tokens: List[int]) -> List[Tensor]:
    """
    Split back a flattened tensor into its original list of tensors with shapes restored.
    """
...
def named_replace(
    ...
    """
    Recursively replace modules in a model by applying a function `fn`.
    """
...
def named_apply(
    ...
    """
    Recursively apply a function `fn` to all modules in a model.
    """
    if not depth_first and include_root:
...
def get_sha() -> str:
    """
    Get git commit SHA, branch, and diff status.
    """
...
def get_conda_env() -> Tuple[Optional[str], Optional[str]]:
    """
    Return current conda environment name and path.
    """
...
def has_batchnorms(model: nn.Module) -> bool:
    """
    Check whether the model has any BatchNorm layers.
    """
