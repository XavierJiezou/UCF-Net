...
import third_party.dinov3.distributed
from third_party.dinov3.eval import results
...
def write_results(results_dict, output_dir, results_filename) -> None:
    ...
    if torch.cuda.is_available() and not third_party.dinov3.distributed.is_main_process():
        return
...
def args_dict_to_dataclass(eval_args: dict[str, object], config_dataclass, save_config: bool = True) -> tuple[Any, str]:
    ...
    if save_config and third_party.dinov3.distributed.is_main_process():
