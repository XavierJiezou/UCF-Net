...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        if self.use_finegrained_image_json():
            image_list, label_list, name_list = self.collect_finegrained_image_json()
            ...
            if self.config.get('with_landmark', False) or self.config.get('with_mask', False):
                if dlib is None:
                    raise ImportError(
                        "dlib is required when with_landmark or with_mask is enabled, "
                        "but it could not be imported in the current environment."
                    )
    ...
    def use_finegrained_image_json(self):
        image_data_config = self.config.get('image_data_json', None)
        return bool(image_data_config and image_data_config.get('enabled', False))

    def get_finegrained_image_json_path(self):
        image_data_config = self.config.get('image_data_json', {})
        split_key = 'train' if self.mode == 'train' else 'val'
        json_path = image_data_config.get(split_key)
        if not json_path:
            raise ValueError(f'image_data_json.{split_key} must be set when image_data_json.enabled is true.')
        return json_path

    def collect_finegrained_image_json(self):
        json_path = os.path.abspath(self.get_finegrained_image_json_path())
        with open(json_path, 'r') as f:
            dataset_info = json.load(f)

        items = dataset_info.get('items')
        if not isinstance(items, list):
            raise ValueError(f'{json_path} must contain an "items" list.')

        image_data_config = self.config.get('image_data_json', {})
        configured_root = image_data_config.get('root')
        metadata_root = dataset_info.get('metadata', {}).get('root')
        root = configured_root or metadata_root
        if not root:
        ...
        label_map = image_data_config.get('label_map', {'real': 0, 'fake': 1})
        image_list, label_list, name_list = [], [], []
        for idx, item in enumerate(items):
            rel_path = item.get('path')
            if not rel_path:
                continue
            image_path = rel_path if os.path.isabs(rel_path) else os.path.join(root, rel_path)

            raw_label = item.get('label')
            if raw_label in label_map:
                label = label_map[raw_label]
            elif raw_label in self.config.get('label_dict', {}):
                label = self.config['label_dict'][raw_label]
            else:
                raise ValueError(f'Label {raw_label} in {json_path} is not supported at item {idx}.')

            image_list.append(image_path)
            label_list.append(int(label))
            name_list.append(item.get('path', image_path))

        if len(image_list) == 0:
            raise ValueError(f'Collect nothing from {json_path}.')

        shuffled = list(zip(label_list, image_list, name_list))
        if self.mode == 'train':
            random.shuffle(shuffled)
        label_list, image_list, name_list = zip(*shuffled)
        return list(image_list), list(label_list), list(name_list)
