...
import argparse
from logger import create_logger, config_to_pretty_dict, get_model_param_report, format_eval_metrics
...
parser.add_argument('--weights_path', type=str,
                    default='./weights/ucfnet_best.pth')
...
def build_dataloader_kwargs(config, batch_size, drop_last=False):
...
def main():
    ...
    run_name = os.path.basename(os.path.dirname(os.path.dirname(weights_path))) if weights_path else 'eval'
    eval_log_dir = os.path.dirname(weights_path) if weights_path else config.get('log_dir', './logs')
    logger = create_logger(os.path.join(eval_log_dir, 'evaluation.log'))
    ...
    if weights_path:
        ...
        logger.info(f"Checkpoint loaded from {weights_path}")
    else:
        logger.info('Fail to load the pre-trained weights')
 ...
 #       print(f"Layer: {layer_name}, Effective Rank: {rank}")
    ...
    for dataset_name, metric_dict in best_metric.items():
        logger.info(format_eval_metrics(dataset_name, metric_dict))
