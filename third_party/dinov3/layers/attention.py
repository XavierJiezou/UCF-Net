...
def rope_apply(x: Tensor, sin: Tensor, cos: Tensor) -> Tensor:
    ...
    # cos: [..., D], eg [cos0, cos1, cos0, cos1, cos2, cos3]
...
class SelfAttention(nn.Module):
    ...
    def compute_attention(self, qkv: Tensor, attn_bias=None, rope=None) -> Tensor:
        ...
        # 手动实现 scaled dot-product attention（替代 torch.nn.functional.scaled_dot_product_attention）
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, is_causal: bool = True) -> Tensor:
        ...
        # 手动实现因果注意力 (Causal Self-Attention)
        attn = torch.matmul(q, k.transpose(-2, -1)) * self.scale  # [B, heads, N, N]
        if is_causal:
